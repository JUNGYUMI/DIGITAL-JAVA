...
-- 임꺽정 학생이 수강했거나 하고있는 과목을 출력하는 SQL
	select st_name, su_title
		from attend
			-- student 테이블에서 일므이 임꺽정인 학생을 검색하여 검색 결과를 st라는 임시 테이블로 생성
			-- MySQL에서는 서브쿼리 결과에 이름을 안붙이면 에러 발생
...
-- 임꺽정 학생이 현재까지 수강했거나 수강하고있는 과목의 총 학점을 출력하도록 SQL문을 작성하세요.
...
-- 2020년 1학기에 대학수학기초 강의 1분반 학생의 수를 확인하는 SQL문
	-- select su_title as '과목명', count(*) as '학생 수'
