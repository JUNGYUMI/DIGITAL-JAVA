...
select * from student where st_name like '홍%'; -- %는 모든 문자열, 홍% = 성이 홍씨, %홍 = 홍으로 끝나는 사람

-- 2020학번인 모든 학생들의 정보를 출력하는 SQL문
select * from student where st_num like '2020%';

-- 데이터 삭세 시 유의사항
-- 삭제하려는 데이터를 다른 테이블에서 외래키로 참조하고있을 때 삭제 불가
-- 1. 참고하고 있는 데이터를 삭제 후 해당(삭제할던) 데이터를 삭제
-- 2. 참고하고 있는 데이터를 수정 후 해당(삭제할던) 데이터를 삭제

-- 학생이 게시글에 첨부파일을 추가하여 게시글을 등록하였다.
-- 이 때 게시글 등록이 성공적으로 이루어지기 위한 전제 조건은 무엇이고, 어느 테이블에서 insert가 일어나야 하는가? (board, attachments 테이블)
...
select * from 
	student 
where st_name like '홍%' && st_num like '2020%';
