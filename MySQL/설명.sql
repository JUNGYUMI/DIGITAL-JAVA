select 속성명1, 속성명2
	...
	select 속성명1, 속성명2
		from 데이터베이스.테이블A
    where 조건;
   /* 데이터베이스 안에 있는 테이블명에서 조건을 만족하는 행중에 속성명1과 속성명2의 값들만 확인 */
   ...
   insert into 테이블A(속성명1, 속성명2, 속성명N)
    ...
    /* 테이블A안에 있는 속성명1에 값1을 저장하고, 속성명2에 값2를 저장하고,
        ...
        2. 속성에 지정된 제약조건에 값이 맞은지 확인
        insert할 때에 생략할 수 없는 속성들
		- 기본키인 속성 
        - 제약조건이 NOT NULL이고, 기본값이 없는 경우 
        insert할 때 속성의 개수와 값의 개수가 같아야 한다. */
	...
	update 테이블A 
		set 
            ...
            속성명N = 값N
	where 조건;
        /* 테이블A 에서 조건을 만족하는 행들의 속성명1을 값1로, 속성명2를 값2로 속성명N을 값 N으로 수정하는 SQL문
        조건에는 기본키를 많이 이용한다.
        */
	...
	delete from 테이블A where 조건;
        ...
        */
        ...
        where절에서 =은 같다를 의미힌다. 
        where절에서 != 이나 <>은 같지않다를 의미한다. 
        where절에서 NULL 체크는 is NULL을 쓰거나 is not NULL을 쓴다. */
        
	DROP TABLE IF EXISTS 테이블A;
        /*테이블 생성 시 기존에 있는 테이블이면 지우고 다시 만들 때
        사용하는 SQL문으로 테이블A가 있으면 삭제, 없으면 아무일도 안함
        (단, 테이블 안에 데이터가 삭제되기 때문에 데이터를 삭제하면 안되는 경우 사용해서는 안됨*/
