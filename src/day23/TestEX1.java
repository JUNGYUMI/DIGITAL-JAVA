...
public class TestEX1 {
	...
	public static void main(String[] args) {
		 ...
		 * 리스트의 size() 메소드는 현재 저장된 객체의 개수
		 * 리스트 안에는 배열로 되어 있는데, 이때 만들어진 배열의 크기가 용량이다.*/
		Scanner scan = new Scanner(System.in);
		List<Integer> list = new ArrayList<Integer>(5);				// <> 안에는 클래스명 넣어준다
		//List<Integer> list = new List<Integer>();	<====== List는 인터페이스이기 때문에 객체를 생성할 수 없다.
		int size = 5;
		...
		int sum = 0;
		double avg = 0.0;
		Iterator<Integer> it = list.iterator();						// 외워두면 편한 칭구
		while(it.hasNext()) {
			/* Iterator를 생성할 때 Iterator의 <>입력을 안하면 Object가 자동으로 들어가고,
			 * 그렇게 되면 it.next()를 통해 가져오는 정보의 클래스도 OBjext가 되기 때문에 형 변환이 필요 */
			Integer tmp = it.next();
